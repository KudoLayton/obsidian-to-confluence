...
export default class Markdown2Confluence {
	...
	preprocessExceptCode(
		source: string,
		functionList: ((a: string) => string)[],
	): string {
		const codeRegex = /(```|`).*?\1/s;
		...
		if (matchResult !== null) {
			...
			assert(
				...
				'Code block index is not number!',
			);
			preCode = source.slice(0, matchIndex);
			postCode = this.preprocessExceptCode(
				source.slice(matchIndex + code.length, source.length),
				functionList,
			);
		} else {
			preCode = source;
	...
	async translateStdin(source: string): Promise<string> {
		const request = new PandocRequest(
			'markdown',
			'jira',
			undefined,
			source,
	...
	async fullTranslate(
		...
		const postProcess = [
			(value: string) =>
				this.attachmentSpaceCharacter(value, attachments),
		];
		const request = new PandocRequest(
			'markdown',
			'jira',
		...
		return this.pandoc
			.executePandocStdin(request)
			.then((value) => this.postprocessExceptCode(value, postProcess));
	}
}
