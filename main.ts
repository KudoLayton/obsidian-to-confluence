import { App, Plugin, Modal, Notice, FileSystemAdapter } from 'obsidian';
  ...
  settings: ConfluencePluginSettings;
	...
	async onload() {
		...
		const ribbonIconEl = this.addRibbonIcon(
			'file-up',
			'Prepare Obsidian to Confluence',
			(_) => {
				const adapter = this.app.vault.adapter;
				if (adapter instanceof FileSystemAdapter) {
					const fileData = this.app.workspace.getActiveFile();
					const md2conf = new Markdown2Confluence(
						this.settings.PandocPath,
						fileData?.parent.path ?? '/',
						adapter,
					);
					...
					if (fileData !== null) {
						const attachments =
							this.app.metadataCache.resolvedLinks[fileData.path];
						for (const attachmentPath in attachments) {
							if (
								...
								attachmentList.push(
									adapter.getFullPath(attachmentPath),
								);
							}
						}
						...
						try {
							md2conf
								...
								.then((transResult) => {
									new ConfluenceWikiView(
										this.app.workspace.getLeaf(
											'split',
											'vertical',
										),
										this.settings,
										fileData?.basename ?? 'noname',
										this.settings.defaultSpace,
										this.settings.defaultParentDocumentID,
										transResult,
										attachmentList,
									);
								});
...
class SampleModal extends Modal {
	...
	constructor(app: App, setting: ConfluencePluginSettings) {
		...
		if (adapter instanceof FileSystemAdapter) {
			const fileData = this.app.workspace.getActiveFile();
			this.md2conf = new Markdown2Confluence(
				this.setting.PandocPath,
				fileData?.parent.path ?? '/',
				adapter,
			);
		}
				...
				this.md2conf
					...
					.then((transResult) => {
						new ConfluenceWikiView(
							...
							this.setting,
							fileData?.basename ?? 'noname',
							this.setting.defaultSpace,
							this.setting.defaultParentDocumentID,
							transResult,
							attachmentList,
						);
					});
